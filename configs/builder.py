...
def BuildConfig(cfg_file_path, tmp_cfg_dir='tmp_cfg'):
    ...
    cfg_dir, cfg_name, base_name = '/'.join(cfg_info.split('/')[:-1]), cfg_info.split('/')[-1], cfg_info.split('/')[-2]
    # base_cfg.py must exist
    base_cfg_file_path = os.path.join(cfg_dir, 'base_' + base_name + ext)
    ...
    shutil.copyfile(base_cfg_file_path, os.path.join(tmp_cfg_dir, 'base_' + base_name + ext))
    time.sleep(0.5)
