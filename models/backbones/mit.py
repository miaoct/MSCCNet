...
class EfficientMultiheadAttention(MultiheadAttention):
    ...
    def forward(self, x, hw_shape, identity=None):
        ...
        if self.batch_first: out = out.transpose(0, 1).contiguous()
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x, hw_shape):
        def _forward(x):
            x = self.attn(self.norm1(x).contiguous(), hw_shape, identity=x)
            x = self.ffn(self.norm2(x).contiguous(), hw_shape, identity=x)
            return x
